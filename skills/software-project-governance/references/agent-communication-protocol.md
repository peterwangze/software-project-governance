...
**Output**:
```
Conclusion: APPROVED | APPROVED_WITH_NOTES | NEEDS_CHANGE | BLOCKED
unresolved_blockers={non-negative integer; APPROVED_WITH_NOTES requires 0}
Findings:
...
每个 Reviewer 的 Output 均为 `Conclusion: APPROVED | APPROVED_WITH_NOTES | NEEDS_CHANGE | BLOCKED`，并输出独立结构字段 `unresolved_blockers={non-negative integer}`。`APPROVED_WITH_NOTES` 必须保留具体备注，且只有 `unresolved_blockers=0` 时才能通过；字段缺失、非零、非法或重复矛盾必须 fail-closed，自然语言中偶然出现的 `blocking` 不构成结构事实。`NEEDS_CHANGES` 仅是兼容输入，不得作为新的输出。未知或格式错误结论必须 fail-closed。Coordinator 收到后 MUST 按下表驱动任务到终态，对应 `behavior-protocol.md` M7.4 step 4.6 闭环状态机：

| Reviewer 结论 | Coordinator MUST 动作 |
|--------------|----------------------|
| APPROVED | 写 REVIEW-{task_id}-R{n} 证据（含 round 号）；任务可标记已完成 |
| APPROVED_WITH_NOTES | 仅在审查输出与 REVIEW-{task_id}-R{n} 证据均含 `unresolved_blockers=0` 且无矛盾时，写证据及非阻塞备注并标记完成；否则 fail-closed |
...
- `conclusion={APPROVED/APPROVED_WITH_NOTES/NEEDS_CHANGE/BLOCKED}`：本轮结论
- `unresolved_blockers={non-negative integer}`：本轮未解决 blocker 数；`APPROVED_WITH_NOTES` 必须为独立结构字段 `unresolved_blockers=0`，缺失、非零、非法或重复矛盾均不得通过
