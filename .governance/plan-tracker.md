...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 项目管理工作流插件 | 维护（并行活跃：规划） | 49 | 42 | 0 | 3 | G11 通过 | 2026-04-24 |
...
| MAINT-011 | 维护 | 双 SKILL 单源化或等价校验 | 消除 `skills/` 与 `.claude/skills/` 两份 SKILL.md 长期漂移风险 | 完整审视结果、两个 SKILL.md、`scripts/verify_workflow.py` | 单源化方案或严格等价校验机制 | Claude | 项目负责人 | 未开始 | P1 | 2026-04-24 | 2026-04-25 | | G8 | 两份 SKILL 的关系被显式定义；校验脚本能发现行为协议漂移，而不只是检查少量关键词 | 待补 | 当前双份 SKILL 仅做片段校验，长期易漂移 | 在校验脚本分层后继续补强 | 由本轮完整审视新增 |
...
| MAINT-024 | 维护 | 企业经验驱动的子工作流优化（首批 4 阶段） | RESEARCH-002 完成了 Google/Amazon/华为/字节实践→阶段映射，但从未执行"用这些经验改造工作流本身"。当前子工作流是通用骨架，未针对 AI 编程项目特征做适配 | RESEARCH-002 企业实践文档、4 个目标阶段的子工作流 | 优化后的立项/架构设计/开发/测试 4 个子工作流（含 Gate 检查项、交互边界标注、产出物标准） | Claude | 项目负责人 | 未开始 | P0 | 2026-04-25 | 2026-05-03 | | G8 | 每个优化后的子工作流能回答：(1) AI 编程场景特有风险是什么；(2) 大公司哪条经验可直接改造为 AI 项目规则；(3) 哪些检查项可自动化判定 | 待补 | 企业经验停在"调研报告"层，未进入"产品改进"层。在反复修架构和路径时，产品内核（内容深度）被搁置 | 先打透 4 个阶段验证模式，再扩展到其余 7 个阶段 | 由 DEC-039 拉回计划，直接服务"解放用户非思考动作"的愿景 |
