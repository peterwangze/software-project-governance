...
- [ ] **M-6 预推校验**：check-release released 模式 + release-ledger `--remote origin` 全绿方进 M-7；UNKNOWN/BLOCKED 不得包装为 PASS
- [ ] **M-7 tag+push**：annotated tag `v0.89.0`（peel = transition 提交；taggerdate 权威——FIX-349；event integrity sha256 双锚）+ master + tag 原子推送（本地/remote peel 一致）
- [ ] **M-8 归档 + 收尾**：`archive.py migrate --auto`（dry-run 先行）+ `check-archive-integrity` PASS + Check 28s 复测（§4.5——专席③后消解有效性确认）+ plan-tracker `工作流版本` → 0.89.0 + roadmap 0.89.0 行 → 已发布 + REL-090 任务行终态回填
