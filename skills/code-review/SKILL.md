...
**Gate semantic:** `loop-exit-gate` for the **Inner loop** (development → test → debug per slice).

This review certifies an Inner loop's **EXIT** — the slice/commit is mergeable. A gate that FAILS does not fail a stage; it returns the work into the Inner loop for another iteration (rework → re-review), incrementing `loop_count`. Only when `loop_count` exceeds the Inner fuse (max_rounds=5) does the failed gate escalate instead of iterating. The development stage contains the entire Inner loop; G6-exit (code-review LGTM) is precisely this exit gate.
