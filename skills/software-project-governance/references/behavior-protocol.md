...
4.6 **Review 闭环状态机（MANDATORY）** ——
   ...
   强制条款（MUST）：
     ...
     (C4) 终态只有两个：APPROVED（✓）或 BLOCKED（✗ → escalation）。"审查拒绝"是中间态，不得作为任务最终状态。
     (C5) round>3 的 APPROVED 仅在 escalation 中用户明确"接受降级"后才允许记为 WARN 终态；否则强制 BLOCKED。
...
| `FX-XXX` | 版本化架构/功能实现切片 | proactive——building new，version-carried，multi-slice + Code Reviewer |
...
**FX vs FIX 消歧**：FIX = reactive repair（修复已有缺陷）；FX = proactive feature slice（构建新能力，有版本承载和切片序列）。
**FX vs REL 消歧**：FX = 实现切片（Slice N of M）；REL = 发布本身（version bump + Release Reviewer）。
**编号规则**：前缀内唯一顺序编号；跨前缀可重叠（FX-187 和 FIX-187 是不同 ID）；引用任务时 MUST 带完整前缀。
