...
从已加载 skill 目录运行 `python "<已加载插件包>/skills/software-project-governance/infra/archive.py" migrate --auto --dry-run` 检查持续归档触发器:
...
→ dry-run 显示需要归档: 运行 `python "<已加载插件包>/skills/software-project-governance/infra/archive.py" migrate --auto`，再运行 `python "<已加载插件包>/skills/software-project-governance/infra/verify_workflow.py" check-archive-integrity`
...
2. **AI Execution Packet 优先读取（0.38.0+）**：
   ...
   — IF 当前任务为活跃 P0/P1 且短包缺失:
     a. 解析 `WORKFLOW_HOME` 后运行 `python "$WORKFLOW_HOME/infra/verify_workflow.py" execution-packet --write`
     b. 再读取生成后的短包继续执行
...
**Hook 存活检测**（系统级约束——不依赖 agent 自觉）：检查 `.git/hooks/pre-commit`、`.git/hooks/commit-msg` 和 `.git/hooks/post-commit` 是否存在。缺失 → ⚠️ 治理 hook 缺失——agent 的 commit 不受系统约束。**MUST** 先解析 `WORKFLOW_HOME`（优先 `SOFTWARE_PROJECT_GOVERNANCE_HOME` / `SPG_HOME`，其次项目内 `skills/software-project-governance`，再查找已安装插件 cache 中包含 `skills/software-project-governance/SKILL.md` 的目录），再提示重装：`cp "$WORKFLOW_HOME/infra/hooks/pre-commit" .git/hooks/pre-commit && cp "$WORKFLOW_HOME/infra/hooks/commit-msg" .git/hooks/commit-msg && cp "$WORKFLOW_HOME/infra/hooks/post-commit" .git/hooks/post-commit`
...
2. **IF** 当前版本 > 记录版本 → 执行以下自动序列：
   ...
   **C. 自动补全 plan-tracker 缺失结构**（agent 自动补全——不是提示，是直接做）：
   ...
   - **自动清理升级残留**（每版本更新时执行）：解析 `WORKFLOW_HOME` 后运行 `python "$WORKFLOW_HOME/infra/cleanup.py"`（基于 manifest.json 的结构 diff——不在 canonical manifest 中的文件 = 残留，自动删除）。输出 `✅ 已清理 {N} 个过期文件/目录`

   **D. 更新 plan-tracker `工作流版本`** 为当前版本

	   **E. 持续归档触发检测与执行**（用户更新插件后自动触发——零用户操作）：
	   — 从已加载 skill 目录运行 `python "<已加载插件包>/skills/software-project-governance/infra/archive.py" migrate --auto --dry-run` 检测四类触发器:
	   ...
	   — dry-run 报告需要归档 → 执行:
	     a. 运行 `python "<已加载插件包>/skills/software-project-governance/infra/archive.py" migrate --auto`
	     b. 运行 `python "<已加载插件包>/skills/software-project-governance/infra/verify_workflow.py" check-archive-integrity`
...
3. agent 的证据可信吗？ → 解析 `WORKFLOW_HOME` 后运行 `python "$WORKFLOW_HOME/infra/verify_workflow.py" check-governance`
...
- 验证命令：解析 `WORKFLOW_HOME` 后运行 `python "$WORKFLOW_HOME/infra/verify_workflow.py"`
...
- **IF** 项目根目录存在 `.git/` → 安装两个 hook：
  ...
  1. **pre-commit hook**（阻断型——commit 前检查）：
     - 解析 `WORKFLOW_HOME` 后复制 `$WORKFLOW_HOME/infra/hooks/pre-commit` 到 `.git/hooks/pre-commit`
  ...
  2. **post-commit hook**（报告型——commit 后检查）：
     - 解析 `WORKFLOW_HOME` 后复制 `$WORKFLOW_HOME/infra/hooks/post-commit` 到 `.git/hooks/post-commit`
     - 每次 `git commit` **之后**自动执行
     - 提取 task ID → 检查 evidence → 输出 check-governance 摘要
     - 不阻断——只报告

  3. **commit-msg hook**（阻断型——消息检查）：
     - 解析 `WORKFLOW_HOME` 后复制 `$WORKFLOW_HOME/infra/hooks/commit-msg` 到 `.git/hooks/commit-msg`
