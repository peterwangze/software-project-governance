...
export function ensurePreset(ctx) {
  ...
  try {
    ...
    //   * `created` is assigned only after that unconditional create returns, so
    //     the cleanup in the catch block can never address a directory this
    ...
    // directory this attempt proved it created (`created` is set only by the
    // successful non-recursive `mkdirSync` above). The former implementation
