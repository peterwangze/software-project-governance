...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 项目管理工作流插件 | 维护（并行活跃：规划） | 92 | 65 | 0 | 3 | G11 通过 | 2026-04-26 |
...
| AUDIT-011 | 维护 | P1: Gate 自动判定脚本（B 级自动化） | 让 verify_workflow.py 能自动判定指定 Gate 的通过/未通过状态，而不只是展示规则文本 | DESIGN-010（Gate 量化检查项）、子工作流定义 | `python scripts/verify_workflow.py gate-check G<N>` 子命令，对指定 Gate 执行自动判定并输出通过/未通过/有条件通过 | Claude | 项目负责人 | 项目负责人 | 未开始 | P1 | 2026-04-28 | 2026-05-02 | | G8 | 至少 G1~G5 的检查项可自动判定；未通过的检查项有明确的缺失说明 | 待补 | 当前 gate 子命令只展示规则，不执行判定。判定完全依赖 agent 手动推理 | 先覆盖可自动判定的检查项（文件存在/片段存在/数字阈值），人工判断项标记为"需确认" | B 级自动化从"脚本展示规则"升级为"脚本执行判定" |
