...
| **0.10.0** | **已发布** | **2026-05-01** | **全角色覆盖 + 治理模型迁移 + 方法论路由** | **AUDIT-063~067(P1) QA/DevOps/Analyst/Release/Maintenance, AUDIT-068(P1) 方法论路由(待完成), AUDIT-069(P1) 提示词工程(待完成), AUDIT-070(P1) 治理模型迁移(待完成), AUDIT-071(P2) profiles 重写(待完成)** | **8角色skill + PUA味道→角色自动匹配(待完成) + risk/decision/evidence任务级粒度(待完成) + Phase-Gate deprecated(待完成)** |
