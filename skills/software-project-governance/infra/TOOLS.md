...
- **输入（实际加载集合，多文件求和——非单文件，拆分文件不能绕过预算）**：
  ...
  - `skill` / `command` 层（按需加载，单独计量，**不并入 resident**）：`skills/software-project-governance/SKILL.md`、`/governance` 命令入口默认载荷（FEAT-038 路由层——拆出的场景/说明文件不在默认注入面，故不计入）。
...
- **输出**：分项表（surface / tier / bytes / chars / CJK / 预算价 token / DSH host 价 token / 状态 / `sha256[:16]` 内容指纹——**漂移锚**：等长改写不改变 token 数，指纹可检）+ 三层 TOTAL + 全域诊断 TOTAL + tokenizer 口径 + "Over budget — gated / report-only" 两行口径 + tool-return 面登记行；`--format json` 输出机读 payload（含每面 `sha256_16`、`tokenizer` 口径字段与 `verdict`）
...
- **预算与裁决**：`--budget-tokens` 默认 6000（**resident hard 硬门**，FEAT-050/DEC-211③ 自切片 A 放宽档翻转；arch 最终目标 4K）。`BUDGET_TIER_POLICY` 逐层声明 gate 姿态：`resident` = hard（EVD-1104 瘦身后三 profile 4,216/5,694/5,966 全 ≤6,000——slice-A 放宽窗已关闭，超限即 issue + FAIL，未来增长 fail-closed）、`skill`/`command` = report-only（测量可见，不参与裁决——⑧ 票独立预算面不并入）。verdict ∈ `PASS` / `ADVISORY` / `FAIL`（当前策略表无 advisory 档，ADVISORY 分支为数据驱动保留臂）；**裁决只由 gated 档决定**——报告与 Check 33 显式分列 "gated over"（动裁决）与 "report-only over"（仅测量），不混称 over-budget（P2-1）；`BUDGET_TIER_POLICY` 缺 `resident` 行时输出显式 issue + FAIL，不是 `KeyError`（fail-closed）。任一 canonical 源解析失败（文件缺失/锚点损坏/切片越界/canonical 源不可解析）→ 显式 issue + `FAIL`（fail-closed，绝不静默记 0）。
