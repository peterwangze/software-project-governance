...
| 代码审查 | Code Reviewer | — | diff 逐项审查 -> 正确性/安全/回归/测试覆盖检查 -> 结论 | 用户触发 | 文件位置、严重级别、事实依据、APPROVED/NEEDS_CHANGE/BLOCKED |
...
- **非 `—` 值**：该任务类型完成后，Coordinator MUST spawn 对应审查 Agent，并在 APPROVED 前将任务标记"待审查"（见 behavior-protocol.md M7.4 step 4.5/4.5b/4.6）。
- **`—`（em-dash）**：该任务自身即为审查类任务（如代码审查、需求审查、设计审查、测试审查、发布审查、复盘审查）或纯协调类任务（任务模糊、需求澄清、性能优化、部署/运维），无后置审查环节。
- **审查终态**：所有非 `—` 任务 MUST 经 M7.4 step 4.6 状态机收敛到 APPROVED（✓）或 BLOCKED（✗ → escalation），不得停留在"审查中"/"审查拒绝"中间态。
...
5. **后置审查列是强制性的**：当"后置审查 Agent(s)"列非 `—` 时，Coordinator MUST 在执行 Agent 完成后 spawn 该审查 Agent。"待审查"状态是产品代码任务完成后的默认状态——只有审查 APPROVED 后任务才能标记"已完成"（见 `behavior-protocol.md` M7.4 step 4.5/4.5b/4.6 审查闭环状态机）。跳过后置审查直接标记完成 = 协议违规。
...
4. 审查 Agent 返回结论后，按 `behavior-protocol.md` M7.4 step 4.6 状态机驱动到终态（APPROVED / BLOCKED）
