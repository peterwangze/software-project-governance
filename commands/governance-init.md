...
3. **IF** bootstrap 版本落后（CLAUDE.md 中非最新模板）→ 自动提示：`运行 /governance-update 升级 bootstrap（不触碰 .governance/ 数据）`
4. 更新 plan-tracker `工作流版本` 为当前版本
5. **用户不需要记住任何命令——每次会话开始自动执行。老用户更新插件后用 /governance-update 升级 bootstrap。**
