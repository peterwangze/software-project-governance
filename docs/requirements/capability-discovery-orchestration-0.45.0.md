...
| FIX-117 | P1 | Restricted-environment benchmark fixtures. | Add fixture-backed host capability context checks for no network, no plugin install, no MCP, no browser, no sub-agent, local skill only, Codex CLI blocked, and Gemini auth blocked; selection trace must produce PASS/BLOCKED/DEGRADED/NOT_SUPPORTED without inventing unavailable capability or treating catalog facts as runtime PASS. | `python -m unittest skills/software-project-governance/infra/tests/test_verify_workflow.py -k CapabilitySelection -v`; `python skills/software-project-governance/infra/verify_workflow.py check-host-capability-context --fail-on-issues`; `python skills/software-project-governance/infra/verify_workflow.py check-governance --fail-on-issues` |
