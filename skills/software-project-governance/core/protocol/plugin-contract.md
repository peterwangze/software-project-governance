...
4. `stages`
   - 每个阶段的子工作流 SKILL 定义，包含进入条件、活动清单、产出物标准、退出条件和 Gate 映射。
5. `stage skills`
...
| stages | `stages` | 通过 `verify_workflow.py` stage mapping 解析到 `skills/stage-*/SKILL.md`，特殊映射：`infrastructure` → `skills/stage-infra/SKILL.md`，`ci-cd` → `skills/stage-cicd/SKILL.md` |
...
- `skills/stage-*/SKILL.md`（同级阶段子工作流 SKILL；具体路径以 `verify_workflow.py` stage mapping 为准）
- `main-workflow.md`（总入口与场景匹配）
- `TOOLS.md`（统一工具索引）

仓库中的 `project/workflows/software-project-governance/` 承载**设计时资产**，不重复本体层内容：
...
- `平台原生入口文件` / `AGENTS.md` 等 bootstrap 指针（无条件前置检查，不依赖 skill 加载状态）
...
## 宿主方承诺：架构不变量 I-1 / I-2 / I-3（DEC-187，永久生效）

用户 2026-09-12 裁定（DEC-187，DEC-188 ② 澄清判据）——本插件对**宿主 agent（dsh 及其它适配宿主）**的承诺，**防回流**：任何后续改造都不得违反，违反即回归为架构缺陷，不是取舍。

- **I-1 禁止任何侵入式修改宿主行为的做法。** 不得覆盖、改写、禁用或"兼容"宿主的任何既有内部行 / 配置 / 默认行为。
- **I-2 禁止让宿主产生对本插件的反向依赖。** 宿主不得因为本插件存在而改变其行为、注册表内容或加载顺序；删除本插件后宿主必须与从未安装时行为一致。
- **I-3 本插件对宿主只允许正向依赖。** 允许 import 其公开包、消费其公开服务、声明 `peerDependencies`；宿主变更时由**本插件侧**适配，绝不通过覆盖宿主内部行来"兼容"。

**机检判据（DEC-188 ②，dsh 适配面）**：安装前后，**既有**宿主行的存在性 / `config` / `disabled` 状态与任何宿主平面注册表内容**零变化**，组合 entry 列表**恰多一行且该行只命名本插件**。
**注意**：判据**不是**"与未安装时逐字节等价"——官方 `dsh plugin add/remove/update` 交付预设的载体就是本插件那一行自有 `- insert:`（`package.json` `dsh.bundle.patch` 声明的 bundle 层）；删掉它即等于"官方安装命令装了不生效"，与用户"官方安装/卸载/升级命令原样可用"的要求冲突。该等价口径已被 DEC-188 明确为不可满足。

**导出禁令（明确记录，防回流）**：
- ❌ 不得 `UPDATE` 任何宿主内部行（如 dsh 的 `agent-presets`、`skill-filesystem`、`tool-skill`）；
- ❌ 不得用 `disabled: false` 推翻部署方（如 `dsh-web-app`）的刻意决定；
- ❌ 不得向宿主平面注册全局 provider / 服务 / 工具；
- ❌ 不得用 `!!js` 解析 `process.argv` 或扫描宿主私有目录（如 `$DSH_HOME/profiles`）做自定位；
- ❌ 不得声明 `system`-trust 的预设根。

**已交付形态（0.80.0 / FIX-310 / commit `5a259e2`，作为合规参照）**：`cordis.patch.yml` = **恰一行自有 `- insert:`**；该行模块 `lib/index.js` 的唯一动作是 `ensurePreset()`（把包内模板渲染为绝对路径写入 `$DSH_HOME/.agent-presets/<id>/`，warn-only、无运行时依赖、staging+rename 幂等）；技能经预设 `customSkillDirs` 渲染包内绝对路径（**单源，不复制 `skills/`**）。反面参照 = 0.80.0 之前的形态（两条 `UPDATE` 打 dsh 内部行 + 三处 `!!js` 自定位），即 RISK-050 的根因。
...
4. **validation 能力**
   - 该 agent 是否能执行 `python skills/software-project-governance/infra/verify_workflow.py` 或等价校验命令？
   - 校验失败后是否能阻止声称"已完成"？
...
- **场景**：用户项目已有 `平台原生入口文件`、`.claude/skills/` 或其他 agent 配置文件，与 workflow 的默认投影冲突。
