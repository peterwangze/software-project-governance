...
解析 `WORKFLOW_HOME` 后运行 `python "$WORKFLOW_HOME/infra/archive.py" migrate --auto --dry-run` 检查持续归档触发器:
...
→ dry-run 显示需要归档: 运行 `python "$WORKFLOW_HOME/infra/archive.py" migrate --auto`，再运行 `python "$WORKFLOW_HOME/infra/verify_workflow.py" check-archive-integrity`
...
2. **IF** 当前版本 > 记录版本 → 执行以下自动序列：
   ...
   **D. 更新 plan-tracker `工作流版本`** 为当前版本
	   ...
	   **E. 持续归档触发检测与执行**（用户更新插件后自动触发——零用户操作）：
	   — 解析 `WORKFLOW_HOME` 后运行 `python "$WORKFLOW_HOME/infra/archive.py" migrate --auto --dry-run` 检测四类触发器:
	   ...
	   — dry-run 报告需要归档 → 执行:
	     a. 运行 `python "$WORKFLOW_HOME/infra/archive.py" migrate --auto`
	     b. 运行 `python "$WORKFLOW_HOME/infra/verify_workflow.py" check-archive-integrity`
