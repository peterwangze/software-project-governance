...
## Agent 职能分组（7 组，13 个文件化角色 Agent + Coordinator 自身）

Agent 按项目运作职能分为 7 组，覆盖从立项到维护的全生命周期。Coordinator 已融入入口 SKILL.md——主 agent 加载后即成为 Coordinator；14 个角色含 Coordinator，13 个文件化角色 Agent 是活跃路由口径，位于 `agents/`。Deprecated 历史参考文件不参与路由口径。

| 职能组 | 目录 | Agent | 生命周期阶段 |
|--------|------|-------|------------|
| **管理组** | —（入口层） | Coordinator | 全流程——统筹调度（已融入入口 SKILL.md） |
| **设计组** | `agents/analyst.md`, `agents/architect.md` | Analyst, Architect | 立项→调研→选型→架构设计 |
| **开发组** | `agents/developer.md` | Developer | 开发实现 |
