...
| G11 | → 下一轮 | passed-with-conditions | 2026-04-20 | 复盘完成、下轮方向明确、版本化记录已更新；遗留项：模板需在 DESIGN-009 闭环后更新 |

## 项目总览

| 项目 | 当前阶段 | 总任务数 | 已完成 | 阻塞中 | 关键风险数 | 最近 Gate 结论 | 最近复盘日期 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 项目管理工作流插件 | 维护（并行活跃：规划） | 35 | 15 | 0 | 6 | G11 有条件通过 | 2026-04-20 |
...
| ID | 阶段 | 任务项 | 目标/预期结果 | 输入 | 输出 | Owner | 协同角色 | 状态 | 优先级 | 计划开始 | 计划完成 | 实际完成 | Gate | 验收标准 | 证据 | 风险/偏差 | 纠偏动作 | 备注 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PLAN-006 | 规划 | 产品内容层实施路线 | 把愿景"解放用户非思考动作"拆成可执行的产品内容任务，明确两条主线的并行节奏 | 新愿景（`DEC-020`）、当前审视报告 | 产品内容层实施路线与子工作流优先级排序 | Claude | 项目负责人 | 未开始 | P0 | 2026-04-20 | 2026-04-22 | | G2 | 明确子工作流优先级、Gate 量化范围、企业经验覆盖计划 | 待补 | 没有路线直接动手会再次发散 | 先对齐路线再执行 | 主线 A 顶层规划，与主线 B 的 PLAN-005 并行 |
| DESIGN-009 | 设计 | 前 4 个阶段子工作流骨架 | 为立项、调研、技术选型、架构设计补 `stages/<stage-id>/sub-workflow.md` | `PLAN-006` 路线、`lifecycle.md` 11 阶段定义 | 4 个 `sub-workflow.md` 文件 | Claude | 项目负责人 | 未开始 | P0 | 2026-04-22 | 2026-04-26 | | G3 | 每个子工作流包含进入条件、活动清单、产出物标准、退出条件；可独立被 agent 读取和执行 | 待补 | 子工作流是"解放用户"的最小单元，缺失则用户无步骤可执行 | 先打透前 4 个关键阶段，验证架构可运行后再扩展 | 选择前 4 个是因为它们覆盖从想法到动手的全链路 |
| DESIGN-010 | 设计 | Gate 量化检查项 | 为 G1~G11 每个检查项补可自动判定的通过/未通过标准 | `stage-gates.md` 现有定性检查项、`DESIGN-009` 子工作流定义 | 更新后的 `stage-gates.md` | Claude | 项目负责人 | 未开始 | P0 | 2026-04-24 | 2026-04-27 | | G3 | 每个检查项有明确的"通过/未通过"判定标准，agent 可自动执行判定 | 待补 | 定性描述 agent 无法自动判定，用户仍需手动判断违背愿景 | 与子工作流同步推进 | Gate 量化是"自动看护"的前提 |
| RESEARCH-002 | 调研 | 企业经验深度补强 | 至少覆盖 4 家企业差异化实践，按阶段映射到具体子工作流步骤 | 现有 `company-practices.md` 6 条通用经验 | 按企业拆解、按阶段映射的经验文档 | Claude | 项目负责人 | 未开始 | P1 | 2026-04-27 | 2026-05-02 | | G2 | 至少 4 家企业、每家有适用条件和具体检查项、映射到子工作流步骤 | 待补 | 当前经验来源太薄，不足以看护真实项目 | 合并原 `MAINT-002` | 替代原 MAINT-002 |
| DESIGN-011 | 设计 | 用户交互边界定义 | 明确"问用户"vs"工作流自己决定"的检查项分类 | `DESIGN-009` 子工作流、`DESIGN-010` Gate 量化 | 每个子工作流标注"需人工判断"和"可自动执行"的检查项 | Claude | 项目负责人 | 未开始 | P1 | 2026-04-28 | 2026-05-01 | | G3 | 每个子工作流的活动清单和 Gate 检查项都有明确的交互边界标注 | 待补 | 不知道什么时候该问用户、什么时候该自动执行 | 依赖子工作流和 Gate 量化完成 | "解放用户"不等于"不问用户" |
| DESIGN-012 | 设计 | 常用 skill/script 清单与首批实现 | 为需求澄清、技术评审、Code Review、发布 checklist、回顾会议补 skill 定义 | `DESIGN-009` 子工作流中的具体操作步骤 | 3~5 个 skill/script 定义文件 | Claude | 项目负责人 | 未开始 | P2 | 2026-05-02 | 2026-05-08 | | G3 | 每个 skill 有明确的触发条件、输入输出和可独立执行能力 | 待补 | 子工作流骨架稳定后再补 | 依赖主线 A 前 4 个任务闭环 | 第三层"具体事务 skill/script 层"的起点 |
