...
description: 加载统一工作流规则、模板、门控和事实源，用于软件项目治理任务。
---

# Software Project Governance — 行为协议

**本文件是行为协议，非参考文档。每条规则均为强制性。**

## 工作流身份

- **id**: software-project-governance
- **version**: 0.10.0
- **目标**: 自动化项目流程管理，让用户专注于思考而非流程
- **supported_agents**: Claude, Codex, Gemini
- **核心能力**: 11 阶段生命周期、11 个 Gate 检查、证据/决策/风险跟踪、3 种触发模式、3 种 profile
...
- 涉及推进或审查项目任务状态
- 涉及 Gate 检查或阶段推进
- 涉及审计执行（`references/audit-framework.md` 中 6 个审计维度的任意一个）
- 涉及补充证据、决策或风险记录
...
当用户请求开发、代码审查、架构设计或任何受益于角色分离的多步骤任务时，agent **MUST** 激活 Agent Team 模式：
...
| 文件 | 读取时机 |
|------|-----------|
| `main-workflow.md` | **始终** — 确认基本目的和匹配规则（位于 skill 根目录，不在 `references/` 中） |
| `TOOLS.md` | 需要查找特定工具/script/checklist 时（位于 skill 根目录，不在 `references/` 中） |
| `references/stage-gates.md` | 执行 Gate 检查时 |
| `references/lifecycle.md` | 进入新阶段时，需要详细阶段定义 |
| `references/profiles.md` | 初始化项目时，用户询问 profile/规模，或切换 profile 时 |
| `references/onboarding.md` | 已有项目接入时（进行中的项目） |
| `references/interaction-boundary.md` | 不确定是自动执行还是询问用户时 |
| `references/audit-framework.md` | 执行 Gate 检查时、完成阶段时、完成 Tier 时（按 DEC-052 分层执行模型）、完成 P0 任务时、重大变更后 |
| `references/agent-failure-modes.md` | 检测到 Agent 行为异常时（协议跳过、选择性执行、虚假闭合、幻觉证据）——排查并执行应急动作 |
| `references/agent-team-architecture.md` | 设计或演进 Agent Team 架构时——Coordinator + 角色 agent + Task-Gate 模型。读取时机：规划架构演进、定义新 agent 角色、从 Phase-Gate 迁移到 Agent Team 模型 |
| `references/user-perspective-principle.md` | **每个任务** — 在完成任何影响用户行为的变更之前，MUST 验证用户可获取性、可发现性和可验证性。在规划/设计新功能时也需加载 |
| `references/data-boundary.md` | 需要澄清用户项目数据与工作流样例数据的边界时——讨论数据隔离或插件部署时读取 |
| `references/agent-entry-differences.md` | 了解 Claude/Codex/Gemini/国内 Agent CLI 入口差异时——扩展多 agent 支持时读取 |
| `references/methodology-routing.md` | Coordinator 按 PUA 味道匹配分发角色 Agent 时——Coordinator 将任务路由给角色 agent 时读取 |

### M2.2 子工作流和技能文件（位于 `stages/`，与本 SKILL.md 同目录）
...
### M2.2b Agent Team — prompt 模板（位于 `agents/` 目录，与本 SKILL.md 同目录）

使用 Agent Team 架构（0.10.0+）时，Coordinator 作为团队负责人。角色 agent 通过 `Agent` 工具使用这些 prompt 模板派生。每个模板遵循 superpowers subagent-driven-development 模式。
...
你 **MUST** 在会话结束时写入 `.governance/session-snapshot.md`。此文件保存跨会话连续性：
...
你 **MUST** 在每次会话结束前使用 AskUserQuestion。至少一个问题，每个问题 2-4 个选项。不以 AskUserQuestion 结束 = 协议违规。
...
**自打断协议（MANDATORY）**：如果你发现自己即将输出内联问题——立即停止。删除问题文本。替换为 AskUserQuestion 工具调用。这不是可选项。最常见的违规模式是以自然语言确认问句结束回复（例如，"要继续吗？""需要我继续吗？""Shall I proceed?"）。这些是 LLM 的对话默认行为，并非来自任何文件的指令——但它们同样是 M5.1 违规。

**为什么 FIX-015 还不够**：FIX-015 清理了源文件污染（子工作流文件中包含 `询问用户："..."` 指令）。但像"要继续吗？"这样的内联问题并非由被污染的文件引起——它们是 LLM 的自然对话模式，在训练数据中出现了数十亿次。没有文件告诉 agent 问"要继续吗？"——这是 LLM 自然结束回复的方式。唯一的防御是预输出自检，在问题到达用户之前捕获该模式。这就是 CLAUDE.md SELF-CHECK 第 4 条存在的原因。
...
**元规则（MANDATORY）**：默认在每个交互边界使用 AskUserQuestion。如果你的回复呈现了需要用户选择下一步行动的信息、标志着一个工作单元的结束或到达了一个自然的决策点——你 MUST 使用 AskUserQuestion。**跳过 AskUserQuestion 是例外，不是规则。** 以下触发映射列出了常见场景，但并非穷尽。有疑问时：问。
...
| P0 任务完成 | "确认 / 修改 / 拒绝" — 按 M7.4 step 5，所有 P0 任务和治理关键文件变更 MUST 在继续前通过 AskUserQuestion 触发交付物审查。跳过此步 = M5 使用不足违规 |
| 交付物需审查 | "确认 / 修改 / 拒绝" |
| 风险处理决策 | "接受 / 缓解 / 转移 / 规避" |
| 技术选型结论 | "确认方案 X 还是 Y"，附推荐理由 |
| 关键决策点 | 按 M5.3 分类——停下来问 |
| 风险升级触发 | "修复 / 记录例外 / 延期" — 当 check-governance 检测到过期风险或已过升级截止日期 |
| 审计发现（阻断/偏差） | "立即修复 / 排程为任务 / 接受风险" — 当审计发现阻断级或偏差级问题时 |
| 破坏性 git 操作（仅 default-confirm 模式） | "Force push / reset --hard / branch -D?" — 按交互边界中的危险操作。非破坏性 git（commit、push、pull）按 M5.3/M5.4 自动执行 |
...
| 场景 | 正确动作 | 为什么可以跳过 |
|----------|---------------|-------------------|
| **执行链中途** — 你正在连续动作序列中（例如，证据 → 验证 → 审计 → commit） | 继续执行。在下一个交互边界使用 AskUserQuestion（例如，commit 之后，继续下一个任务之前） | 用户已确认方向；中间步骤不需要重新确认 |
...
**关键判断**："我的回复是否向用户呈现了关于下一步做什么的选择？" 如果是 → AskUserQuestion。如果用户需要决定、确认或选择 → 你 MUST 使用 AskUserQuestion。唯一跳过的情况是你在已确认的执行链中，且当前步骤不需要用户输入。
...
- 执行 Gate 检查时读取 `references/stage-gates.md` 获取详细 Gate 检查项
...
**风险升级截止日期强制执行**（AUDIT-045）：已过升级截止日期（risk-log "截止日期" 列）的开放风险 **MUST** 升级——要么关闭并给出解决方案，要么升级到更高严重级别。风险在截止日期已过的情况下保持"打开"状态是协议违规。`check-governance` Check 8 检测截止日期已过的开放风险。

**任务截止日期强制执行**（AUDIT-048）：已过"计划完成"日期的活跃任务（"未开始"或"进行中"）**MUST** 升级——要么完成，要么重新规划新截止日期，要么明确降级。`check-governance` Check 9 检测计划完成日期已过的任务。
...
2. **运行外部验证** — `python scripts/verify_workflow.py check-governance`。按 M8.1，脚本验证可捕获 agent 自检无法发现的结构性问题。
3. **自审计** — 如果任务是 P0 级别，或修改了任何治理关键文件（SKILL.md、stage-gates.md、audit-framework.md、verify_workflow.py、lifecycle.md、profiles.md、onboarding.md、interaction-boundary.md、agent-failure-modes.md、main-workflow.md、TOOLS.md），执行 D1（目标对齐）+ D4（变更闭合）审计维度，参考 `references/audit-framework.md`。将审计结论记录到 evidence-log。
4. **交付物审查（在 commit 之前 — P0/治理关键任务 MANDATORY）** — 如果完成的任务是 P0 优先级或修改了任何治理关键文件（SKILL.md、stage-gates.md、audit-framework.md、verify_workflow.py、lifecycle.md、profiles.md、onboarding.md、interaction-boundary.md、agent-failure-modes.md、main-workflow.md、TOOLS.md、pre-commit-hook.sh、post-commit-hook.sh、governance-init.md、CLAUDE.md）→ **MUST** 使用 AskUserQuestion 向用户展示交付物以供审查。**AskUserQuestion 就是此步骤的输出。** 不要在此之前输出独立摘要或分析——摘要内容存在于 AskUserQuestion 的正文中。问题正文包含所做工作的简要摘要并请用户确认。选项："确认——继续 commit" / "修改——需要改动" / "拒绝——回滚"。**审查在 commit 之前：commit 是通过审查的奖励，而非跳过审查的触发器。** 跳过 P0 任务的此审查 = M5 使用不足违规（Failure Mode 11）。对于非 P0、非治理关键的任务 → 进入步骤 5。
5. **Commit** — `git add` 变更的治理文件并 `git commit`，commit message **MUST** 包含任务 ID 作为前缀（例如，"AUDIT-044: 描述"、"MAINT-028: 描述"）。按 DEC-025，每个有意义的变更都是一个 commit 单元。任务完成就是一个 commit 边界。任务 ID 前缀是代码变更与 plan-tracker 条目之间的链接——没有它，可追溯性就断了。不含任务 ID 前缀的 commit message 可被 check-governance Check 7 检测到。
...
**为什么审查在 commit 之前（Failure Mode 11 的结构性修复）**：M5 使用不足重复出现了 7 次，因为旧的顺序（commit → 审查）制造了一个结构性陷阱：commit 提供了认知闭合，而之后的摘要满足了"告诉用户发生了什么"的冲动。独立摘要与 AskUserQuestion 争夺终端位置——摘要总是胜出，因为它更简单且不需要等待输入。两个结构性变更打破了这个陷阱：(1) 审查移到 commit 之前，使 commit 成为通过审查的奖励而非跳过审查的触发器；(2) 摘要嵌入在 AskUserQuestion 内部——没有独立的摘要输出。AskUserQuestion 就是交付物审查。这消除了摘要作为竞争性终端动作的可能性，使审查成为通向闭合的唯一路径。

**协议设计原理**：AUDIT-040 事件中检测到的 4 项违规（未触发审计、未自动 commit、执行中断、使用内联问题而非 AskUserQuestion）均映射到任务完成序列中缺失的步骤。通过使序列显式且原子化，协议消除了导致每项违规的缺口。M7.4 序列已重构为审查把关 commit 而非跟随——确保未经审查的变更永远不会被提交。
...
4. **在所有 commit 中引用任务 ID** — 每个 commit message **MUST** 包含任务 ID 作为前缀（例如，"AUDIT-044: 描述"）。按 DEC-025 和 M7.4 step 4。commit message 中的任务 ID 是代码变更与 plan-tracker 条目的链接——没有它，可追溯性就断了。

**修改文件而没有对应的 plan-tracker 条目 = 协议违规。** 未跟踪的修改会破坏项目的可追溯性——没有任务 ID，证据无法链接，Gate 检查无法验证完成，项目 Owner 无法重建变更原因。每个文件修改 MUST 可追溯到 plan-tracker 中的恰好一个任务。

**为什么存在此协议**：AUDIT-043（M7.4 修复）在被添加到 plan-tracker 之前就已经实施了——修改了 8 个文件，更新了 5 个版本声明，但没有任务条目存在。该任务在代码已经提交之后才被追溯创建为 AUDIT-043。M7.5 通过要求任务条目先存在来防止这种模式。AUDIT-044 是第一个遵循此协议的任务——任务条目先提交（04d0b7c），然后才开始任何实现。
...
**独立脚本验证**通过 `python scripts/verify_workflow.py check-governance` 执行 10 项 agent 无法造假的检查：
...
| 6. Tier 审计完整性 | 已完成 Tier 无 TIER-X-Y-AUDIT 证据 | Agent 可能跳过 Tier 审计，且 agent 自检无法检测（与 Check 5 同为自审计矛盾） |
| 7. Commit-任务可追溯性 | Commit message 中无任务 ID 引用 | Agent 可能在未先创建 plan-tracker 条目的情况下修改文件——未跟踪的修改会破坏可追溯性 |
| 8. 风险升级截止日期 | 已过升级截止日期的开放风险 | Agent 无内置截止日期感知——风险截止日期默默过期 |
| 9. 任务截止日期强制执行 | 已过计划完成日期的活跃任务 | 与 Check 8 相同的模式——任务截止日期存在但从未被强制执行 |
| 10. M5 AskUserQuestion 合规 | 源文件中包含 `询问用户` 反模式（教 agent 输出内联问题）；bootstrap 缺少 AskUserQuestion 规则；interaction-boundary.md 缺少 AskUserQuestion 绑定 | Agent 读取包含字面 `询问用户："..."` 指令的子工作流文件并按字面执行——输出内联文字而非使用 AskUserQuestion。Agent 自检无法检测，因为 agent 正在遵循它读到的（已被污染的）指令。静态反模式检测在 agent 遇到污染之前捕获根因 |
...
**会话结束协议**（M4.2）**MUST** 包含运行 `python scripts/verify_workflow.py check-governance` 作为验证步骤。如果 check-governance 报告问题，**MUST** 在会话结束前修复。
...
详细阶段定义请读取 `references/lifecycle.md`。
...
**MUST 保留**（项目根路径，非相对于本 SKILL.md）：`./protocol/`、`./workflows/software-project-governance/`、`./scripts/verify_workflow.py`

**可以移除**：`./CLAUDE.md`、`./adapters/claude/`
