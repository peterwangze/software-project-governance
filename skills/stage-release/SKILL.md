...
| 活动 | 描述 | 交互边界 | 企业实践 |
|------|------|---------|---------|
| 执行发布 checklist | 逐项检查：测试通过、门禁 green、版本声明一致、适配层状态真实、归档闭环、交叉引用无漂移 | **自动**：运行 `python skills/software-project-governance/infra/verify_workflow.py check-release`；该命令默认执行 `verify`、`check-governance --fail-on-issues`、`e2e-check` 和 unittest；需要检查本机 agent runtime 时追加 `--runtime-adapters`；发布版本已写入 CHANGELOG 后追加 `--version X.Y.Z --require-changelog` | [华为 IPD] 发布门控逐项检查，不可跳过任何一项 |
...
- [ ] 发布脚本检查已通过：`verify_workflow.py check-release` PASS（默认包含 `verify`、`check-governance --fail-on-issues`、`e2e-check`、unittest）；需要真实 agent runtime 佐证时 `--runtime-adapters` PASS；正式发布时 `--version X.Y.Z --require-changelog` PASS
