...
**禁止行为**：不得在用户未确认 profile 和 trigger_mode 的情况下直接创建 .governance/ 目录。不得使用"默认 standard + always-on"静默初始化。
...
- **always-on**（默认）→ 执行完整 Step 1~4。治理面板可正常输出。
- **on-demand** → 仅执行 Step 1。Step 2~4 仅在用户显式调用 governance 命令时执行。**MUST NOT** 主动输出治理面板或流程建议。
- **silent-track** → 执行 Step 1~2，但 **MUST NOT** 向用户输出治理面板、风险统计、任务进度表。仅在 Gate 失败或风险 escalation 到期时打断用户。用户感知不到工作流存在——除非关键节点被触发。

**每次会话输出一句确认**（所有模式均输出）：
> 🔍 Governance active — stage: {stage}, Gate {gate}: {status}, {active_risk_count} active risk(s)
...
**工作流脱轨检测**：检查 plan-tracker 的 `最近复盘日期`——如果距今 > 7 天 AND 有若干新 commit 但 plan-tracker 无更新 → ⚠️ 工作流可能已被忽略。提醒用户："上次治理活动距今 {N} 天。是否继续沿用当时的阶段状态，还是需要更新？"
...
**IF** 用户请求直接进入开发/测试/发布等后期阶段，但当前 Gate 状态显示前置 Gate 均为 pending →
**MUST** 警告用户："当前项目处于 {current_stage} 阶段（{current_gate} pending）。你确定要直接进入 {requested_stage} 吗？跳过前置阶段可能导致：(1) 没有明确目标就开始编码的返工风险；(2) 没有架构设计导致后续重构成本高；(3) 缺少 Gate 检查的证据记录。建议至少完成 {current_gate} 检查后再推进。"

**这不是阻止用户前进——是确保用户知悉风险。** 用户可以选择继续跳过，但决策必须记录到 decision-log。

### Step 4: 优先级确认
passed-with-conditions 遗留项或有 P0 任务 → 优先处理。上次 session 未完成的 P0 任务 → 继续执行。

**没读 plan-tracker 就开始干活 = 流程违规。跳过阶段跳跃防护 = 流程违规。**

### 干活前检查（每次收到任务时）
- 这个任务在计划跟踪表里吗？不在就先入账（创建 task ID + 填必填字段）
- 做完后需要补什么证据？先想清楚
...
- 范围变更 / 架构决策 / 发布决策 / 风险接受 / 外部依赖变更 / Profile或触发模式变更 / 阶段跳跃（跳过 Gate）
...
5. **自动 git commit**（commit message 必须引用 task ID）
