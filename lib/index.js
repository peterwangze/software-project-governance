...
/**
 * The degraded contract view: nothing substituted, the declared leftover scan
 * absent. Every template token is then reported as a leftover by
 * `renderComposition()`'s fallback scan, and `ensurePreset()` skips the sync.
 */
...
function contractDocument() {
  if (contractCache === undefined) {
    ...
    try {
      ...
      if (parsed !== null && typeof parsed === 'object') contractCache = parsed
    } catch {
      contractCache = null
    }
  }
 ...
 * that cannot be read yields an empty binding set: no substitution happens and
 * every token is reported as a leftover — the caller then skips the sync
 * instead of writing a broken preset. Nothing is thrown.
 *
...
export function renderComposition(template, pkgRoot) {
  ...
  // that a fixed known-token list cannot see.
  const leftovers = text.match(bindings.leftoverScan) || []
