...
| FIX-016 | 维护 | M5.1 运行时违规修复——预输出自拦截机制 | 用户反馈 (2026-04-30)——FIX-015 后 agent 仍输出"要继续吗？"内联问题。根因：FIX-015 是静态源头修复，但内联问题是 LLM 对话习惯，不是文件教的。缺失预输出扫描 | (1) CLAUDE.md SELF-CHECK 新增第4项 M5 预输出自检 (2) governance-init 模板同步 (3) SKILL.md M5.1 "STOP and replace"指令 (4) agent-failure-modes.md 失败模式10 (5) verify_workflow.py Check 10 新增 SELF-CHECK 覆盖检测 | Claude | 项目负责人 | 项目负责人 | 已完成 | P0 | 2026-04-30 | 2026-04-30 | 2026-04-30 | G11 | Check 10: 4/4 PASSED + SELF-CHECK 含 M5 第4项 + 失败模式10 | EVD-134 | FIX-015 盲区：清理了文件污染但没解决对话习惯——"要继续吗？"不是文件教的，是 LLM 自然收尾模式。CONSTRAINT-001 设计哲学在 M5 层面：agent 不会自觉遵守规则，但文本输出无法被 git hook 阻断——唯一的系统级强制力是预输出自检 | 版本 0.7.1→0.7.2 |
