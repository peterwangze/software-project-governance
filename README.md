...
Both forms were re-verified this way (temp-dir `DSH_HOME`, zero writes to the real home) on dsh 0.1.2-rc.1 + pnpm 11.22.0, 2026-09-05 — all verification ran on Windows; non-Windows is unverified〔isolation: link:/file: 2026-09-05, Windows-only〕〔static: github: form — packaging semantics by reasoning, not install-verified (see RISK-049)〕. Note on `github:`: `v0.79.0` is tagged locally but **not yet pushed** (push credentials blocked — disclosed in `docs/release/release-checklist-0.79.0.md`), and `v0.80.0` is a **candidate awaiting authorization** (no tag exists; `release_authorized=false`, tag/push pending the M-4 gate — `docs/release/release-checklist-0.80.0.md`). GitHub's master therefore still serves 0.78.1; install from a local checkout (`link:`/`file:`) or wait for the push.
