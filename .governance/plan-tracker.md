...
| FIX-018 | 维护 | M7.4 结构修复——review移到commit之前+summary嵌入AskUserQuestion | M5 under-use 复发7次深度根因——summary与AskUserQuestion结构性竞争 | (A)summary嵌入AskUserQuestion (B)审查移到commit之前。新顺序: evidence→verify→audit→AskUserQuestion审查→commit→continue | Claude | 项目负责人 | 项目负责人 | 进行中 | P0 | 2026-04-30 | 2026-04-30 | | G11 | verify PASSED + M7.4重构 + 失败模式11根因更新 | 待补 | 5层文本规则修复全部失效——都只是"加更多规则"没改变"summary总是赢"的结构 | — |
