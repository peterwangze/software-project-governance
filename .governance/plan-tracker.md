...
| --- | --- | --- | --- | --- | --- | --- | --- |
| 项目管理工作流插件 | 维护（并行活跃：规划+架构） | 123 | 114 | 0 | 2 | G11 通过 | 2026-04-30 |
...
| FIX-017 | 维护 | M5 under-use 修复——AskUserQuestion 触发点静默跳过 | 用户反馈 (2026-04-30)——FIX-016 完成后 agent 直接总结+停止，没用 AskUserQuestion 确认交付物。M5.2 "Deliverable needs review" 触发点被 M7.2 "stopping after one task prohibited" 覆盖 | (1) M7.4 新增 step 5: P0/治理关键文件交付物 MUST AskUserQuestion 审查 (2) M5.2 新增触发点: "P0 task completion" (3) 失败模式11: M5 under-use | Claude | 项目负责人 | 项目负责人 | 进行中 | P0 | 2026-04-30 | 2026-04-30 | | G11 | SKILL.md M7.4 step 5 + M5.2 P0 触发 + 失败模式11 | 待补 | M7.2 "禁止停止" vs M5.2 "要求审查" 冲突——agent 将禁止停止解释为禁止所有停止，包括必要的审查停止 | 快速通道 |
...
| AUDIT-053 | 维护 | 全规则一致性审计修复——32 项矛盾/死规则/漂移/引用错误闭环 | 上一 session 全规则审计 (2026-04-30) 发现 32 个问题：8 严重矛盾 + 7 重要矛盾 + 4 引用错误 + 7 死规则 + 1 版本鸿沟 + 3 漂移 + 2 M5 合规。需全量修复后方可推进 0.8.0 | (P0) C1-C8 8 严重矛盾修复 (P1) S1-S7 7 重要矛盾修复 (P2) R1-R4/D1-D7/V1/DR1-DR3/AQ1-AQ2 17 项修复 | Claude | 项目负责人 | 项目负责人 | 进行中 | P0 | 2026-04-30 | 2026-04-30 | | G11 | verify PASSED + 32 项修复记录在 evidence + 版本 bump 0.7.1→0.7.2 | 待补 | 长期演进中规则层积累了结构性不一致——M7.2 vs M7.4 step 5 是典型：一个规则禁止停止，另一个规则要求停止 | 0.8.0 前置质量门 |
