...
2.6 **变更影响分析**（仅产品代码）——
   ...
   IF P0 任务且涉及 ≥2 个架构层的修改 → MUST 额外 spawn Analyst + Architect
      **系统强制**: pre-commit hook Step 10-12 验证 `目标对齐:` 和 `用户影响:` 字段。缺失 → BLOCK commit。紧急情况可用 `--no-verify` 绕过。
...
1. **文件目标无重叠校验**：对每个即将 spawn 的 agent，提取其任务描述中明确指定的文件路径（或由 Coordinator 从任务范围推断）。任意两个 agent 的文件修改目标有交集 -> **MUST NOT** 并行 spawn——改为串行执行或使用 worktree 隔离。

2. **预检豁免**：仅读取文件（不修改）的 agent 之间，或只读 agent 与修改 agent 之间，无冲突风险——可并行。

3. **自检条目**：M8 自检 checklist 新增：`- [ ] M7.6 并行 agent 文件目标无重叠？`
...
- [ ] M7.6 并行 agent 文件目标无重叠？（Coordinator spawn >=2 agent 时已预检？）
