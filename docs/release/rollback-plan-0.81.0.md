...
- **状态**：**M-1 草稿**（2026-09-13）——行为清单已确定；`V8` 交付项与最终 commit 清单待其落地后回填（标记 `⟦待 V8 回填⟧`）。
...
| **消费方改造（行为保持）** | `lib/index.js`（契约绑定 + 惰性化 + JS `schema_version` fail-closed）、`infra/dsh_compat.py`（零校验不得 PASS + group 语义对齐 + 惰性绑定）、`adapters/dsh/launch.py`（渲染/解码守卫 + `DSH_HOME` 收敛 + 写入守卫对称化） | **需重装预设**——见 §2.2（渲染产物 sha256 不变，但代码路径变化） |
...
# 2) 回退到本版之前的稳定点（二选一）
#    (a) 精确回退本版提交区间（推荐，保留历史）
git -C <plugin_root> revert --no-commit <0.81.0 提交区间…>   # ⟦待 V8 回填区间⟧
#    (b) 直接切回稳定 tag
...
3. 校验：`agent.cordis.yml` 的 sha256 应等于 `00e0d330f3560e10381b54b37a06fe6a626a451e3e70da8126b9c2bc47be3723`（**本版与 0.80.0 的渲染产物字节相同**——这是本版"行为保持"的硬基线，故回滚**不应**改变预设内容）。
...
| 5 | 三路径渲染 sha256 | `00e0d330…3723`（回滚前后一致） |
...
*草稿结束（M-1）。`⟦待 V8 回填⟧` 项在 V8 落地后补齐，随后作为 M-2 门禁「回滚方案」的交付物参与 `check-release`。*
