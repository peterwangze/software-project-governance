...
> @bootstrap-version: 0.88.0（模板最低引导版本——低于 SKILL frontmatter active_version 即陈旧，先升级本段再继续）
...
> @bootstrap-version: 0.88.0（薄指针版——FEAT-037 双入口去重；完整 bootstrap 见 {PRIMARY_ENTRY}（主入口），行为约束以主入口为准）
