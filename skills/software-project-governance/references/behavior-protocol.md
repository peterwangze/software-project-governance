...
### M5.1c 附件输入通道 — 固定回退选项 + 对话框应答回合合法化（FIX-396）

AskUserQuestion 的结构化选项无法承载图片/附件/长自由文本。当问题需要用户以上述形态作答时，**M5.1 不豁免**——禁止改为内联文字索图（"请把截图发我"仍是违规问句），问题本身 MUST 仍经 AskUserQuestion 呈现。合法路径是**固定回退选项 + 对话框应答**：

1. **固定回退选项（MUST）**：选项中包含一个固定回退项，推荐措辞「📷 通过聊天框发送图片/材料」，选项描述注明「选择后请在对话框直接发送」。回退选项 MUST NOT 设为 recommended/首位默认——避免诱导用户绕过选项化决策。
2. **对话框应答回合合法化（MUST）**：用户选择回退选项后，其随后在对话框（正常聊天输入框）发送的图片/消息构成该问题的**合法应答回合**——不判内联违规、不破坏交互边界。M5.1 约束的是 agent 输出问句的渠道，不约束用户主动发送的材料。Coordinator 正常消费应答回合：图片按平台能力原生查看，或按需路由视觉 agent（DSH 下经 `route_agent` vision/附件通道）。
3. **通道边界（MUST）**：本通道仅限 AskUserQuestion 无法承载的输入形态（图片/附件/长自由文本）。可选项化的关键决策（范围/架构/发布/风险/模式，M5.3）仍 MUST 选项化呈现——不得借本通道让自由文本替代结构化决策；能写成 2-4 个选项的问题不适用本通道。

用户回合侧的边界定义见 interaction-boundary.md「对话框应答回合（FIX-396）」——两侧条款互为镜像，任何一方变更 MUST 同步另一方。
