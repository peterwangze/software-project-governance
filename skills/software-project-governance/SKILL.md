...
本工作流按六层架构组织（详见 `references/four-layer-architecture.md`）：

```
适配层（平台投影）→ 入口层（本文件）→ 业务智能层（Agent 库）→ 能力层（SKILL 库）→ 基础设施层 → 核心层
```

- **适配层**：`adapters/` + `.claude-plugin/` + `.codex-plugin/` + `CLAUDE.md`——平台原生格式投影
- **入口层**：本文件——加载 Coordinator Agent
- **业务智能层**：`agents/`——9 个角色 Agent（persona + 判断 + 多 SKILL 编排）
...
| 文件 | 用途 |
|------|------|
| `references/four-layer-architecture.md` | 六层架构设计（本工作流的结构设计） |
...
> 仓库根目录的 `CLAUDE.md` 不是产品资产——它是当前仓库使用 Claude Code 开发的临时文件。
