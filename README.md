...
0.54.1 packages the governance hook hotfix release scope for FIX-140, but it is not the 1.0.0 release. It versions nested plugin/workflow product path detection and commit-msg dated evidence row matching: product hooks recognize root product paths and nested plugin/workflow product paths while excluding `.governance/**`, and commit evidence matching supports both `EVD | TASK_ID` and `EVD | date | TASK_ID` row shapes. 0.54.1 is a semver patch over 0.54.0; it does not change the already reviewed hook repair logic beyond release metadata strings. `classic-phase-gate` remains the active/default compatibility preset, `dynamic-flow-gate` remains inactive/non-default, and 0.54.1 does not change 0.55.0 Dynamic Lifecycle migration/external validation planning, does not migrate projects, does not make dynamic-flow-gate the default, does not change registry automation command execution, and does not declare 1.0.0 readiness. RISK-036 and RISK-037 remain open: no official approval, no marketplace approval, no two-real-project external validation full PASS, no Codex Desktop lifecycle PASS, no project migration, no RISK-036 closure, no RISK-037 closure, and no 1.0.0 production-ready claim.
