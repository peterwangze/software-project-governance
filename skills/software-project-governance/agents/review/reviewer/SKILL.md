...
1. 读取任务指定的审查 SKILL 文件（见下方 SKILL 绑定表）——按 SKILL 定义的审查维度逐项执行
2. 逐行审查 → AI 专项检查 → 安全检查 → 设计一致性检查 → 标注严重级别 → 输出审查结论
3. 完成后返回结构化结论给 Coordinator:
   - 完成状态（APPROVED / NEEDS_CHANGE / BLOCKED）
   - 审查发现的完整列表（文件:行号 / 严重级别 / 问题描述 / 修复建议）
   - 证据（审查报告路径或结构化输出）
