...
检查 `.git/hooks/pre-commit` 和 `.git/hooks/post-commit` 是否存在。缺失 → ⚠️ 治理 hook 缺失——agent 的 commit 不受系统约束。**MUST** 提醒重装：`cp scripts/pre-commit-hook.sh .git/hooks/pre-commit && cp scripts/post-commit-hook.sh .git/hooks/post-commit`

**版本变化自动检测 + bootstrap 自升级**（用户更新插件后下次会话自动触发——零用户行动）：
1. 对比 plan-tracker `工作流版本` 与当前安装版本
2. **IF** 当前 > 记录 → agent 自动执行：
   - 输出更新摘要（版本跨度 + CHANGELOG 要点）
   - **自动替换 CLAUDE.md 中的 bootstrap 段为最新模板**（保留其余内容）
...
当需要修改 bootstrap 行为时：
```
❌ 禁止：CLAUDE.md → 改完 → "同步到" governance-init.md
       狗粮实例不能是事实源——改了用户得不到

✅ 强制：governance-init.md Step 7 模板 → bump 版本 →
       用户 /plugin update → bootstrap 自升级 →
       本仓库 CLAUDE.md 通过自升级同步
       模板是唯一事实源——用户通过插件更新获得
```

- **MUST NOT** 直接修改 CLAUDE.md 添加新行为。CLAUDE.md 是本仓库狗粮实例。
...
- **IF** 项目根目录存在 `.git/` → 安装两个 hook：
  ...
  1. **pre-commit hook**（阻断型——commit 前检查）：
     - 复制 `scripts/pre-commit-hook.sh` 到 `.git/hooks/pre-commit`
  ...
  2. **post-commit hook**（报告型——commit 后检查）：
     - 复制 `scripts/post-commit-hook.sh` 到 `.git/hooks/post-commit`
...
已创建文件:
  ...
  {if bootstrap_injected}✅ CLAUDE.md — 治理 bootstrap 已注入
  {if not bootstrap_injected}⊘ CLAUDE.md — 治理 bootstrap 已存在，已跳过
