...
运行 `python skills/software-project-governance/infra/archive.py migrate --auto --dry-run` 检查持续归档触发器:
...
→ dry-run 显示需要归档: 运行 `archive.py migrate --auto`，再运行 `verify_workflow.py check-archive-integrity`
...
2. **AI Execution Packet 优先读取（0.38.0+）**：
   ...
   — IF 当前任务为活跃 P0/P1 且短包缺失:
     a. 运行 `python skills/software-project-governance/infra/verify_workflow.py execution-packet --write`
     b. 再读取生成后的短包继续执行
   ...
   **D. 更新 plan-tracker `工作流版本`** 为当前版本
	   ...
	   **E. 持续归档触发检测与执行**（用户更新插件后自动触发——零用户操作）：
	   ...
	   — dry-run 报告需要归档 → 执行:
	     ...
	     b. 运行 `python skills/software-project-governance/infra/verify_workflow.py check-archive-integrity`
...
- **IF** 项目根目录存在 `.git/` → 安装两个 hook：
  ...
  3. **commit-msg hook**（阻断型——消息检查）：
     - 复制 `skills/software-project-governance/infra/hooks/commit-msg` 到 `.git/hooks/commit-msg`
