...
- **口径**: `source CLI proxy + target fixture checks`。subprocess 命令在 source root 执行 CLI 代理（当前脚本硬编码 ROOT）；`project/e2e-test-project/` 通过 tracked fixture checks 验证，不伪称完整外部 cwd 执行。
...
| `project/e2e-test-project/.governance/plan-tracker.md` | `工作流版本` / `操作权限模式` / `default-confirm` | target_fixture |
