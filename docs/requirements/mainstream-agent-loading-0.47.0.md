...
| --- | --- | --- |
| Tier 1 loading guide | Codex, Claude Code, Gemini CLI, opencode, Chrys | Provide direct README and adapter guidance, validation commands, status boundary, and no-overclaim wording. |
...
| Chrys | Chrys agent platform (internal runtime — `https://github.com/peterwangze/software-project-governance/blob/master/adapters/chrys/README.md`) | Chrys auto-loads `AGENTS.md` and `CLAUDE.md` as native context files; has native `load_skill` / `read_skill_resource` / `run_skill_script` tools for skill consumption; native `ask_user` tool for user interaction; native sub-agents (`explore_agent` / `plan_agent` / `general_agent`) for Agent Team routing. | Keep Chrys adapter (`adapters/chrys/`) with full adapter-manifest, launcher, and README. Chrys has the strongest native capability profile: native ask_user_question, sub_agent, tool_calling, and git_hooks. Only browser and MCP remain host-dependent. E2E verified 2026-07-01 from live Chrys session. |
...
| Chrys | `AGENTS.md` + `CLAUDE.md` native context loading, `adapters/chrys/*`, native skill loading via `load_skill` | PASS / DEGRADED (2026-07-01) | Adapter added in post-0.47.0 maintenance cycle. Chrys has the strongest native capability profile of any adapter; only browser and MCP remain host-dependent. |
