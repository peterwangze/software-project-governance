...
> @bootstrap-version: 0.78.1（模板最低引导版本——低于 SKILL frontmatter active_version 即陈旧，先升级本段再继续；无法确定新版本时不升级，输出 /plugin update 指引——FIX-238.2 fail-closed）
...
- "当前模式" / "现在什么模式" → 输出当前 trigger_mode x permission_mode
...
- **用户视角三问**：1 用户怎么获得变更（update/init/手动？）2 用户怎么知道变更存在？3 用户体验真的变了吗？
