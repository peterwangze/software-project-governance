# Rollback Plan - 0.53.0

**Version**: 0.53.0

Rollback is required if the REL-033 release package activates declarative gate behavior, migrates projects, makes `dynamic-flow-gate` default, changes classic G1-G11 behavior, closes RISK-036/RISK-037 without evidence, or overclaims official approval, marketplace status, external validation full PASS, Codex Desktop lifecycle PASS, or 1.0.0 readiness.

## Triggers

- `check-release --version 0.53.0 --require-changelog --runtime-adapters` fails.
- `check-version-consistency` reports a 0.53.0 declaration mismatch.
- `check-manifest-consistency --fail-on-issues` reports missing 0.53.0 release docs coverage.
- `check-lifecycle-registry --fail-on-issues` reports project-type preset, classic compatibility, or dynamic-flow-gate default drift.
- `classic-phase-gate` is no longer active/default.
- `dynamic-flow-gate` becomes active/default in 0.53.0.
...
1. Revert the 0.53.0 package commit after it exists:

```bash
git revert <0.53.0-release-package-commit>
```
...
3. Re-prepare 0.53.0 with the preset-data-only and no-overclaim boundary below before retrying REL-033.
