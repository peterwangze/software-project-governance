...
1. **下一个最优先事项**：[从样例跟踪表中找当前最高优先级的未开始任务]
2. **待关闭的遗留项**：[列出 passed-with-conditions 的未关闭项]
3. **需要用户判断的问题**：[列出需要用户思考或决策的开放问题]
4. **风险提醒**：[列出当前打开的高严重级别风险]
```

这个建议的目的是让用户不需要自己翻看项目状态，直接知道下一步该做什么、该想什么。用户只负责思考和决策，工作流负责整理和推荐。
