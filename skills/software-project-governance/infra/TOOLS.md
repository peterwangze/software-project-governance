...
- **预算与裁决**：`--budget-tokens` 默认 6000（**切片 A 放宽档**；arch 最终目标 4K）。`BUDGET_TIER_POLICY` 逐层声明 gate 姿态：`resident` = advisory（当前标准/严格档入口模板超 6K——其瘦身是独立排期任务，本档只报告不阻断）、`skill`/`command` = report-only（测量可见，不参与裁决）。verdict ∈ `PASS` / `ADVISORY` / `FAIL`；**裁决只由 gated 档决定**——报告与 Check 33 显式分列 "gated over"（动裁决）与 "report-only over"（仅测量），不混称 over-budget（P2-1）；`BUDGET_TIER_POLICY` 缺 `resident` 行时输出显式 issue + FAIL，不是 `KeyError`（fail-closed）。任一 canonical 源解析失败（文件缺失/锚点损坏/切片越界/canonical 源不可解析）→ 显式 issue + `FAIL`（fail-closed，绝不静默记 0）。
