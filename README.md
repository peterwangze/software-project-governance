...
0.55.0 packages the Dynamic Lifecycle migration preview and external validation archive for FIX-139, VAL-005, and VAL-006, but it is not the 1.0.0 release. It publishes the dry-run-only `dynamic-lifecycle-migration --target <path> --dry-run` preview, the migration guide, the `python_game` chapter-flow validation archive, and the `shitu` non-game validation archive. `classic-phase-gate` remains the active/default compatibility preset, `dynamic-flow-gate` remains inactive/non-default and opt-in only, and no project is migrated by this release. The two external validations are intentionally conservative: both dry-run previews reached `READY_FOR_REVIEW`, but installed-state validation still has target-native blockers, and non-game preset generalization remains partial because `shitu` preview flow units still come from the `python_game_10_chapters` example. RISK-036 and RISK-037 remain open: no official approval, no marketplace approval, no two-real-project external validation full PASS, no Codex Desktop lifecycle PASS, no project migration, no RISK-036 closure, no RISK-037 closure, and no 1.0.0 production-ready claim.
