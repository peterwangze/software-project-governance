...
**Gate 语义：** 开发切片的 Inner loop 的 `loop-exit-gate`（开发 → 测试 → 调试）。本审查认证 Inner loop 的**退出条件**：当前切片或提交可以合并；开发阶段包含完整的 Inner loop，G6-exit（代码审查 LGTM）即为该退出门。

审查失败不会终止阶段；它会将工作返回所属循环（Inner loop）继续迭代（返工 → 复审），并递增 `loop_count`。只有当 `loop_count` 超过 Inner fuse（`max_rounds=5`）时，失败才升级而不是继续迭代。
