...
0.54.3 packages the governance same-package skill fast-start hotfix scope for FIX-144, but it is not the 1.0.0 release. It versions the corrected `/governance` entry model so users can update the plugin and have the loaded command use the same plugin package's `skills/software-project-governance` skill directly before any repo-local or diagnostic fallback. `SOFTWARE_PROJECT_GOVERNANCE_HOME`, `SPG_HOME`, and `WORKFLOW_HOME` are diagnostic fallback clues only, not startup prerequisites. 0.54.3 is a semver patch over 0.54.2; it does not change Dynamic Lifecycle migration scope. `classic-phase-gate` remains the active/default compatibility preset, `dynamic-flow-gate` remains inactive/non-default, and 0.54.3 does not change 0.55.0 Dynamic Lifecycle migration/external validation planning, does not migrate projects, does not make dynamic-flow-gate the default, does not change registry automation command execution, and does not declare 1.0.0 readiness. RISK-036 and RISK-037 remain open: no official approval, no marketplace approval, no two-real-project external validation full PASS, no Codex Desktop lifecycle PASS, no project migration, no RISK-036 closure, no RISK-037 closure, and no 1.0.0 production-ready claim.
