...
### 提问规则（强制）
**AskUserQuestion 是唯一合法的用户提问方式。** 禁止内联文字问"要不要继续""是否如何如何"——所有需要用户回答的问题 MUST 通过 AskUserQuestion 工具。轻量项目也不例外。

**必须问的关键决策**（停下来用 AskUserQuestion）：
- 范围变更 / 架构决策 / 发布决策 / 风险接受 / 外部依赖变更 / 阶段跳跃
...
**Sub-agent 调度规范**：
- 每个子 agent 启动时 MUST 加载两个文件：角色定义（`agents/<name>.md`）+ 任务规范（`skills/<skill-name>/SKILL.md`）
- Sub-agent 硬边界：可读取文件/执行审查/返回结构化结果，不可与用户交互（无 AskUserQuestion）/修改治理状态/做最终决策/与其他 Sub-agent 直接通信

**调度模板（强制）**：Coordinator spawn sub-agent 时 MUST 使用 `skills/software-project-governance/references/agent-dispatch-template.md`——禁止传自定义 prompt，只能填充模板中的占位符。

**行为协议**：详见 `skills/software-project-governance/references/behavior-protocol.md`（M0-M9 强制性规则）——所有角色 Agent 必须遵守。
...
**Hook 存活检测**（系统级约束——不依赖 agent 自觉）：
检查 `.git/hooks/pre-commit` 和 `.git/hooks/post-commit` 是否存在。缺失 → ⚠️ 治理 hook 缺失——agent 的 commit 不受系统约束。**MUST** 提醒重装：`cp skills/software-project-governance/infra/hooks/pre-commit .git/hooks/pre-commit && cp skills/software-project-governance/infra/hooks/post-commit .git/hooks/post-commit`

**版本变化自动检测 + bootstrap 自升级**（用户更新插件后下次会话自动触发——零用户行动）：
1. 对比 plan-tracker `工作流版本` 与当前安装版本
2. **IF** 当前 > 记录 → agent 自动执行：
   - 输出更新摘要（版本跨度 + CHANGELOG 要点）
   - **自动替换 平台原生入口文件 中的 bootstrap 段为最新模板**（保留其余内容）
   - 自动补全 plan-tracker 缺失结构（版本规划/需求跟踪/变更控制含快速通道/项目配置字段）
   - 检查 hook 存活并提示安装命令
   - **自动清理升级残留**：运行 `python skills/software-project-governance/infra/cleanup.py`（基于 manifest.json 的结构 diff——不在 canonical manifest 中的文件 = 残留，自动删除）。输出 `✅ 已清理 {N} 个过期文件/目录`
   - 更新 `工作流版本`
3. **用户要做的全部：/plugin update → 下次会话 → 自动完成。**

### Step 1.5: 开发纪律——bootstrap 变更的正确流向（MANDATORY — 工作流开发者 MUST 遵守）

**本段仅供工作流开发者参考——终端用户不需要关心。**

当需要修改 bootstrap 行为时：
```
❌ 禁止：平台原生入口文件 → 改完 → "同步到" governance-init.md
       狗粮实例不能是事实源——改了用户得不到

✅ 强制：governance-init.md Step 7 模板 → bump 版本 →
       用户 /plugin update → bootstrap 自升级 →
       本仓库 平台原生入口文件 通过自升级同步
       模板是唯一事实源——用户通过插件更新获得
```

- **MUST NOT** 直接修改 平台原生入口文件 添加新行为。平台原生入口文件 是本仓库狗粮实例。
- **MUST** 先改 `commands/governance-init.md` Step 7 注入模板。

### Step 2: 交叉验证（3 项强制检查）
1. **证据完整性**：plan-tracker 中"已完成"的任务，evidence-log 中是否有对应证据？缺失 → **检查 profile**：lightweight 不强制证据（信息提示），standard/strict = P0 漏洞。
...
**IF** 用户请求跳过前置 Gate → **MUST** 通过 AskUserQuestion 警告（M5.1 禁止内联文字）。选项："继续跳过—已知悉风险"/"先完成当前 Gate 检查"。跳过 MUST 记录 decision-log。
...
### 干活前检查（每次收到任务时）
- **任务入账检查**：这个任务在 plan-tracker 里吗？不在 → **先入账再动手**——哪怕只有 ID+一句话描述（快速通道 30s）。用户临时插入的任务也不例外。
- 做完后需要补什么证据？先想清楚
- 这个任务会不会影响别的阶段？影响就先记风险
- **跨任务检查**：上一个任务证据补齐了吗？没补齐 → 先补再开新任务。不允许带着证据债进入下一个任务。
- **用户视角三问**：①用户怎么获得变更 ②用户怎么知道变更存在 ③用户体验真的变了吗
