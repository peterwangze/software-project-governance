...
## Loop semantic / identity 授权边界（0.66.2）

- semantic claim gate 的 `PASS` 仅证明历史 claim ownership 与当前语义扫描通过，不构成 release identity 授权。
- 在独立的三根 identity attestation 与 aggregate equality 通过前，聚合状态必须保持 `IDENTITY_ATTESTATION_PENDING`。
- `UNKNOWN`、`NOT_APPLICABLE`、缺失证据或 unexpected skip 均不得把 pending 状态提升为授权状态。
