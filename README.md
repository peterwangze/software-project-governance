...
0.54.0 packages the Declarative Gate Engine release scope for classic G1-G11 gate judgment, but it is not the 1.0.0 release. It versions FIX-138 classic registry-backed gate execution: `auto_judge_gate()` now reads classic gate definitions from lifecycle registry `gate_execution_registry`, while automation commands remain metadata and classic G1-G11 compatibility remains the active/default runtime path. `classic-phase-gate` remains the active/default compatibility preset, `dynamic-flow-gate` remains inactive/non-default, and 0.54.0 does not migrate projects, does not make dynamic-flow-gate the default, does not execute registry automation commands as part of gate judgment, and does not declare 1.0.0 readiness. RISK-036 and RISK-037 remain open: no official approval, no marketplace approval, no two-real-project external validation full PASS, no Codex Desktop lifecycle PASS, no RISK-036 closure, no RISK-037 closure, and no 1.0.0 production-ready claim.
