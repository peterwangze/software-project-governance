# Rollback Plan - 0.54.0

**Version**: 0.54.0

Rollback is required if the REL-034 release package migrates projects, makes `dynamic-flow-gate` default, executes registry automation commands as part of gate judgment, changes the release boundary for RISK-036/RISK-037, or overclaims official approval, marketplace status, external validation full PASS, Codex Desktop lifecycle PASS, or 1.0.0 readiness.

## Triggers

- `check-release --version 0.54.0 --require-changelog --runtime-adapters` fails.
- `check-version-consistency` reports a 0.54.0 declaration mismatch.
- `check-manifest-consistency --fail-on-issues` reports missing 0.54.0 release docs coverage.
- `check-lifecycle-registry --fail-on-issues` reports gate execution registry, classic compatibility, automation metadata, or dynamic-flow-gate default drift.
- `classic-phase-gate` is no longer active/default.
- `dynamic-flow-gate` becomes active/default in 0.54.0.
- Registry `automation_command` entries are executed during gate judgment.
...
1. Revert the 0.54.0 package commit after it exists:

```bash
git revert <0.54.0-release-package-commit>
```
...
3. Re-prepare 0.54.0 with the classic-registry-execution and no-overclaim boundary below before retrying REL-034.
